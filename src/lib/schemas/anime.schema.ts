...
export const createAnimeSeriesSchema = z.object({
  ...
  anilistId: z.number().positive('AniList ID pozitif bir sayı olmalı'),
  idMal: z.number().positive('MAL ID pozitif bir sayı olmalı').optional(),
  type: z.nativeEnum(AnimeType),
  status: z.nativeEnum(AnimeStatus),
  episodes: z.number().min(1, 'Bölüm sayısı en az 1 olmalı').optional(),
  duration: z.number().min(ANIME.DURATION.MIN, 'Süre en az 1 dakika olmalı').optional(),
  isAdult: z.boolean(),
  season: z.nativeEnum(Season).optional(),
  seasonYear: z.number().min(ANIME.YEAR.MIN, 'Yıl 1900-2100 arasında olmalı').max(ANIME.YEAR.MAX, 'Yıl 1900-2100 arasında olmalı').optional(),
