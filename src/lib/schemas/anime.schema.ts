...
import { baseUrlSchema } from './shared/base';

// Anime Series oluşturma şeması
export const createAnimeSeriesSchema = z.object({
  title: z.string().min(ANIME_DOMAIN.VALIDATION.TITLE.MIN_LENGTH, 'Anime başlığı gerekli').max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'Anime başlığı çok uzun'),
  englishTitle: z.string().max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'İngilizce başlık çok uzun').optional(),
  nativeTitle: z.string().max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'Yerel başlık çok uzun').optional(),
  synopsis: z.string().max(ANIME_DOMAIN.VALIDATION.SYNOPSIS.MAX_LENGTH, 'Özet çok uzun').optional(),
  ...
  trailer: baseUrlSchema.optional().or(z.literal('')),
...
export const updateAnimeSeriesSchema = z.object({
  title: z.string().min(ANIME_DOMAIN.VALIDATION.TITLE.MIN_LENGTH, 'Anime başlığı gerekli').max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'Anime başlığı çok uzun').optional(),
  englishTitle: z.string().max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'İngilizce başlık çok uzun').optional(),
  nativeTitle: z.string().max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'Yerel başlık çok uzun').optional(),
  synopsis: z.string().max(ANIME_DOMAIN.VALIDATION.SYNOPSIS.MAX_LENGTH, 'Özet çok uzun').optional(),
...
export const createAnimeMediaPartSchema = z.object({
  ...
  title: z.string().min(ANIME_DOMAIN.VALIDATION.TITLE.MIN_LENGTH, 'Medya parçası başlığı gerekli').max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'Medya parçası başlığı çok uzun'),
  displayOrder: z.number({ required_error: 'İzleme sırası gerekli' }).min(1, 'İzleme sırası gerekli'),
  notes: z.string().max(ANIME_DOMAIN.VALIDATION.SYNOPSIS.MAX_LENGTH, 'Notlar çok uzun').optional(),
  episodes: z.number().min(ANIME_DOMAIN.VALIDATION.EPISODES.MIN, 'Bölüm sayısı en az 1 olmalı').max(ANIME_DOMAIN.VALIDATION.EPISODES.MAX, 'Bölüm sayısı çok yüksek').optional(),
  anilistId: z.number().optional(),
  malId: z.number().optional(),
  anilistAverageScore: z.number().min(ANIME_DOMAIN.VALIDATION.SCORE.MIN).max(ANIME_DOMAIN.VALIDATION.SCORE.MAX).optional(),
  anilistPopularity: z.number().min(ANIME_DOMAIN.VALIDATION.POPULARITY.MIN).optional(),
});

// Anime Media Part güncelleme şeması
export const updateAnimeMediaPartSchema = z.object({
  title: z.string().min(ANIME_DOMAIN.VALIDATION.TITLE.MIN_LENGTH, 'Medya parçası başlığı gerekli').max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'Medya parçası başlığı çok uzun').optional(),
  displayOrder: z.number({ required_error: 'İzleme sırası gerekli' }).min(1, 'İzleme sırası gerekli'),
  notes: z.string().max(ANIME_DOMAIN.VALIDATION.SYNOPSIS.MAX_LENGTH, 'Notlar çok uzun').optional(),
  episodes: z.number().min(ANIME_DOMAIN.VALIDATION.EPISODES.MIN, 'Bölüm sayısı en az 1 olmalı').max(ANIME_DOMAIN.VALIDATION.EPISODES.MAX, 'Bölüm sayısı çok yüksek').optional(),
  anilistId: z.number().optional(),
...
export const createEpisodeSchema = z.object({
  ...
  title: z.string().max(ANIME_DOMAIN.VALIDATION.TITLE.MAX_LENGTH, 'Bölüm başlığı çok uzun').optional(),
  description: z.string().max(ANIME_DOMAIN.VALIDATION.SYNOPSIS.MAX_LENGTH, 'Açıklama çok uzun').optional(),
  ...
  fillerNotes: z.string().max(ANIME_DOMAIN.VALIDATION.SYNOPSIS.MAX_LENGTH, 'Filler notları çok uzun').optional(),
});
...
export const animeFiltersSchema = z.object({
  ...
  limit: z.number().min(ANIME_DOMAIN.PAGINATION.MIN_PAGE).max(ANIME_DOMAIN.PAGINATION.MAX_LIMIT).default(ANIME_DOMAIN.PAGINATION.DEFAULT_LIMIT),
});
