...
export async function createEmailVerificationToken(
  ...
  try {
    ...
    const expiresAt = new Date(Date.now() + EMAIL_VERIFICATION_TOKEN_EXPIRY_HOURS * (60 * 1000))
    ...
    // Email gönder
    const verificationUrl = `${baseUrl}${ROUTES.PAGES.AUTH.VERIFY_REQUEST}?token=${token}`
...
export async function createPasswordResetToken(
  ...
  try {
    ...
    const expiresAt = new Date(Date.now() + PASSWORD_RESET_TOKEN_EXPIRY_HOURS * (60 * 1000))
    ...
    // Email gönder
    const resetUrl = `${baseUrl}${ROUTES.PAGES.AUTH.RESET_PASSWORD}?token=${token}`
...
export async function verifyPasswordResetToken(token: string): Promise<ApiResponse<{ email: string }>> {
  try {
    ...
    logError(LOG_EVENTS.AUTH_PASSWORD_RESET_FAILED, 'Şifre sıfırlama kontrolü hatası', {
      error: error instanceof Error ? error.message : 'Bilinmeyen hata',
      tokenPrefix: token.substring(0, 8) + '...'
    })
...
export async function resetPasswordWithToken(
  ...
  try {
    ...
    await sendPasswordChangedNotification({
      to: user.email,
      username: user.username || user.email.split('@')[0],
      changeTime: new Date().toLocaleString('tr-TR')
    })
